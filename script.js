        ...
        function toggleCategory(categoryId) {
            ...
            if (categoryCommandsDiv && toggleIcon) {
                ...
                toggleIcon.classList.toggle('rotated'); // Rotate icon when collapsed/expanded
            }
        }
        ...
        function displayPopularCommands() {
            ...
            sortedCategoryNames.forEach((categoryName, index) => {
                const categoryId = `category-${index}`; // Unique ID for each category's command list

                // Create category heading with toggle icon
                const categoryHeading = document.createElement('h3');
                categoryHeading.className = 'category-heading';
                categoryHeading.setAttribute('data-target', categoryId); // Link heading to command div
                categoryHeading.onclick = () => toggleCategory(categoryId); // Add click event to toggle

                // Use the right arrow icon (rotated when expanded)
                ...
                // Create container for commands within this category
    ...
    // Função para exibir detalhes do comando
    function displayCommandDetails(command) {
        commandDetailsContainer.innerHTML = `
            <h3 class="text-3xl font-bold text-[#FC6D20] mb-3">${command.name}</h3> <!-- Laranja Itaú para título de comando -->
            <p class="text-lg mb-6">${command.description}</p>
        `;
        ...
        command.examples.forEach(example => {
            ...
            if (example.resultTable) {
                exampleDiv.innerHTML += `
                    <h5 class="text-xl font-medium text-[#003060] mt-4 mb-2">Tabela Resultante:</h5> <!-- Azul escuro Itaú para o título da tabela resultante -->
                    ${renderTable(example.resultTable.name || 'Resultado', example.resultTable.headers, example.resultTable.rows)}
                `;
            } else if (example.afterStateTable) {
                 exampleDiv.innerHTML += `
                    <h5 class="text-xl font-medium text-[#003060] mt-4 mb-2">Estado da Tabela Após o Comando:</h5> <!-- Azul escuro Itaú para o título da tabela pós-comando -->
                    ${renderTable(example.afterStateTable.name, example.afterStateTable.headers, example.afterStateTable.rows)}
                `;
            }
    ...
    // Esconder resultados da pesquisa ao clicar fora
    ...
    // Inicializar a exibição de comandos populares ao carregar a página
    window.onload = displayPopularCommands;
